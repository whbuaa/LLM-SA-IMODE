...
%--------------------------------------------------------------------------
    ...
    methods(Access = protected, Sealed)
        ...
        global   record  
            ...
            run_inter  = run_inter+1;     
            record(run_inter,1)     = Population.best.obj;  
            if strcmp(class(obj.pro),'F7')
                optimum = 910;
            else
                optimum = 0;
            end
            if (run_inter>10000)&& abs((record(run_inter,1)-record(run_inter-10000+1,1)<1e-50))
                nofinish    = false;
            end
            if (Population.best.obj==optimum)
                nofinish    = false;
            end
%             nofinish = obj.pro.FE < obj.pro.maxFE;
